...
│       ├── core/
│       │   ├── GameManager.ts     (208行) 中央调度器，持有 Room，事件总线
│       │   ├── GameEvents.ts      (10行)  事件名常量
│       │   ├── ServerActions.ts   (70行)  19 个已封装的服务器事件
│       │   └── DisplayText.ts     (52行)  角色/技能名称映射
│       ├── scenes/
│       │   ├── HomeScene.ts       (278行) 主页：创建/加入房间、房间列表
│       │   ├── LobbyScene.ts      (240行) 大厅：角色选择、技能选择
│       │   ├── BattleScene.ts     (318行) 战斗：目标选择、骰子、行动
...
│  ServerActions — 类型安全的 emitAck 封装 (23个事件)       │
...
| 文件 | 行数 | 用途 |
|---|---|---|
| `models/ViewModels.ts` | 148 | 展示层类型：SeatVM, DicePanelVM, ActionSlotVM, RoguelitePanelVM 等 |
| `helpers/BattlePlayerHelpers.ts` | 218 | 纯函数库：getActor, canTarget, canLocalAct, hpPercent, playerStatus, buildSeatTags 等 |
| `helpers/DiceAnimator.ts` | 161 | 骰子动画状态机：idle→fast→slow→pause→reveal→idle |
| `helpers/EmoteHelper.ts` | 108 | 表情管理：6 个表情定义、冷却控制、自动过期 |
| `managers/AuthManager.ts` | 155 | REST API 认证：register/login/logout/me/updateProfile/refreshCharacters |
...
│   ├── GameEvents.ts         # 事件名常量
│   ├── ServerActions.ts      # 23 个服务器事件封装 (修改: +8个事件)
